#!/bin/bash
# Eureka 服务管理脚本
# 使用方法: ./eureka.sh {start|stop|restart|status}
# 部署目录: /opt/xuyou
APP_NAME="xuyou-eureka"
APP_JAR="xuyou-eureka-0.0.1.jar"
APP_PORT=8090
# 部署目录配置
APP_HOME="${APP_HOME:-/opt/xuyou}"
JAR_DIR="${JAR_DIR:-$APP_HOME/jar}"
LOG_DIR="${LOG_DIR:-$APP_HOME/log/eureka}"
PID_FILE="$APP_HOME/deploy/eureka.pid"
...
start() {
    ...
    # 设置日志文件路径
    LOG_FILE="$LOG_DIR/xuyou-eureka-all.log"
    
    # JVM参数配置（2核2G服务器优化）
    # Eureka服务相对轻量，分配256-512MB内存
    JAVA_OPTS="-Xms256m -Xmx512m -XX:+UseG1GC -XX:MaxGCPauseMillis=200"
    JAVA_OPTS="$JAVA_OPTS -XX:MetaspaceSize=128m -XX:MaxMetaspaceSize=256m"
    JAVA_OPTS="$JAVA_OPTS -XX:+HeapDumpOnOutOfMemoryError"
    JAVA_OPTS="$JAVA_OPTS -XX:HeapDumpPath=$LOG_DIR/heapdump-eureka.hprof"
