...
CREATE TABLE IF NOT EXISTS `user_points_log` (
    ...
    `points_type` ENUM('SIGN_IN', 'TASK', 'PURCHASE', 'CONSUME', 'ADMIN_ADJUST', 'REFUND', 'INCREASE', 'DECREASE', 'OTHER') NOT NULL COMMENT '积分类型',
...
-- 补签卡库存表
CREATE TABLE IF NOT EXISTS `sign_in_repair_card` (
	`card_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '补签卡ID',
	`user_id` BIGINT NOT NULL COMMENT '用户ID',
	`card_type` TINYINT NOT NULL DEFAULT 1 COMMENT '补签卡类型: 1-普通补签卡, 2-高级补签卡(可补签多天)',
	`quantity` INT NOT NULL DEFAULT 0 COMMENT '库存数量',
	`source` ENUM('MONTHLY_GRANT', 'PURCHASE', 'REWARD', 'ADMIN_GRANT') DEFAULT 'MONTHLY_GRANT' COMMENT '来源: MONTHLY_GRANT-每月发放, PURCHASE-购买, REWARD-奖励, ADMIN_GRANT-管理员发放',
	`expiry_date` DATE DEFAULT NULL COMMENT '过期日期(为空表示永久有效)',
	
	`created_at` DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建时间',
	`updated_at` DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新时间',
	
	PRIMARY KEY (`card_id`),
	UNIQUE KEY `idx_user_type` (`user_id`, `card_type`),
	KEY `idx_user_id` (`user_id`),
	KEY `idx_expiry_date` (`expiry_date`),
	CONSTRAINT `fk_sign_in_repair_card_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '补签卡库存表';

-- 补签记录表
CREATE TABLE IF NOT EXISTS `sign_in_repair_record` (
	`record_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '补签记录ID',
	`user_id` BIGINT NOT NULL COMMENT '用户ID',
	`repair_date` DATE NOT NULL COMMENT '补签日期',
	`card_type` TINYINT NOT NULL DEFAULT 1 COMMENT '使用的补签卡类型',
	`continuous_days_before` INT NOT NULL DEFAULT 0 COMMENT '补签前连续签到天数',
	`continuous_days_after` INT NOT NULL DEFAULT 0 COMMENT '补签后连续签到天数',
	`points_reward` INT NOT NULL DEFAULT 0 COMMENT '补签获得的积分',
	`status` ENUM('SUCCESS', 'REVOKED') NOT NULL DEFAULT 'SUCCESS' COMMENT '状态: SUCCESS-成功, REVOKED-已撤销',
	
	`created_at` DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建时间',
	`updated_at` DATETIME(6) DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新时间',
	
	PRIMARY KEY (`record_id`),
	UNIQUE KEY `idx_user_repair_date` (`user_id`, `repair_date`),
	KEY `idx_user_id` (`user_id`),
	KEY `idx_repair_date` (`repair_date`),
	KEY `idx_status` (`status`),
	CONSTRAINT `fk_sign_in_repair_record_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '补签记录表';

-- 补签卡发放记录表
CREATE TABLE IF NOT EXISTS `sign_in_repair_card_grant` (
	`grant_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '发放记录ID',
	`user_id` BIGINT NOT NULL COMMENT '用户ID',
	`card_type` TINYINT NOT NULL DEFAULT 1 COMMENT '补签卡类型',
	`quantity` INT NOT NULL COMMENT '发放数量',
	`source` ENUM('MONTHLY_GRANT', 'PURCHASE', 'REWARD', 'ADMIN_GRANT') NOT NULL COMMENT '来源: MONTHLY_GRANT-每月发放, PURCHASE-购买, REWARD-奖励, ADMIN_GRANT-管理员发放',
	`grant_month` VARCHAR(7) DEFAULT NULL COMMENT '发放月份(YYYY-MM), 用于月度发放追踪',
	...
	KEY `idx_user_id` (`user_id`),
	KEY `idx_grant_month` (`grant_month`),
	KEY `idx_source` (`source`),
	CONSTRAINT `fk_sign_in_repair_card_grant_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '补签卡发放记录表';

-- 补签配置表
CREATE TABLE IF NOT EXISTS `sign_in_repair_config` (
	`config_id` INT NOT NULL AUTO_INCREMENT COMMENT '配置ID',
...
) ENGINE = INNODB AUTO_INCREMENT = 1 DEFAULT CHARSET = utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT = '补签配置表';

-- 邮箱验证Token记录表
CREATE TABLE IF NOT EXISTS `email_validate_tokens` (
    `token_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'Token记录ID',
    `user_id` BIGINT NOT NULL COMMENT '关联用户ID(关联users.user_id)',
    `email` VARCHAR(100) NOT NULL COMMENT '待验证邮箱地址',
    `token` VARCHAR(100) NOT NULL COMMENT '验证Token(唯一标识)',
    `token_type` ENUM('REGISTER', 'RESET', 'BIND', 'OTHER') NOT NULL DEFAULT 'REGISTER' COMMENT 'Token类型：REGISTER-注册验证, RESET-重置密码, BIND-绑定邮箱, OTHER-其他',
    `status` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '状态：0-未使用, 1-已验证, 2-已过期, 3-已撤销',
    `expire_at` DATETIME(6) NOT NULL COMMENT 'Token过期时间',
    `validated_at` DATETIME(6) DEFAULT NULL COMMENT '验证通过时间',
    `revoked_at` DATETIME(6) DEFAULT NULL COMMENT '撤销时间',
    ...
    KEY `idx_user_id` (`user_id`),
    KEY `idx_email` (`email`),
    KEY `idx_token_type_status` (`token_type`, `status`),
    KEY `idx_expire_at` (`expire_at`),
    CONSTRAINT `fk_email_tokens_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE

) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='邮箱验证Token记录表';

-- 邮箱验证历史记录表
CREATE TABLE IF NOT EXISTS `email_validate_history` (
    `history_id` BIGINT NOT NULL AUTO_INCREMENT COMMENT '历史记录ID',
    `user_id` BIGINT DEFAULT NULL COMMENT '关联用户ID(可为空，未注册用户)',
    `email` VARCHAR(100) NOT NULL COMMENT '邮箱地址',
    `action_type` ENUM('SEND', 'VALIDATE', 'EXPIRE', 'REVOKE') NOT NULL COMMENT '操作类型：SEND-发送, VALIDATE-验证成功, EXPIRE-过期, REVOKE-撤销',
    `token_type` ENUM('REGISTER', 'RESET', 'BIND', 'OTHER') DEFAULT 'REGISTER' COMMENT 'Token类型',
    `result` TINYINT(1) NOT NULL DEFAULT 0 COMMENT '操作结果：0-失败, 1-成功',
    ...
    KEY `idx_created_at` (`created_at`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='邮箱验证历史记录表';
...
-- 用户补签卡概览视图
CREATE OR REPLACE VIEW `user_repair_card_view` AS
SELECT 
    u.user_id,
    u.username,
    COALESCE(SUM(CASE WHEN src.card_type = 1 AND (src.expiry_date IS NULL OR src.expiry_date >= CURDATE()) THEN src.quantity ELSE 0 END), 0) AS normal_card_count,
    COALESCE(SUM(CASE WHEN src.card_type = 2 AND (src.expiry_date IS NULL OR src.expiry_date >= CURDATE()) THEN src.quantity ELSE 0 END), 0) AS advanced_card_count,
    COALESCE(SUM(CASE WHEN src.expiry_date IS NULL OR src.expiry_date >= CURDATE() THEN src.quantity ELSE 0 END), 0) AS total_card_count,
    MAX(src.updated_at) AS last_updated
FROM users u
LEFT JOIN sign_in_repair_card src ON u.user_id = src.user_id
GROUP BY u.user_id, u.username;

-- 用户补签记录详情视图
CREATE OR REPLACE VIEW `user_repair_record_view` AS
SELECT 
    srr.record_id,
    srr.user_id,
    u.username,
    ...
    srr.card_type,
    srr.continuous_days_before,
    srr.continuous_days_after,
    srr.points_reward,
    srr.status,
    srr.created_at
FROM sign_in_repair_record srr
LEFT JOIN users u ON srr.user_id = u.user_id;

-- 用户补签卡发放历史视图
CREATE OR REPLACE VIEW `user_repair_card_grant_view` AS
SELECT 
    srcg.grant_id,
    srcg.user_id,
    u.username,
    ...
    srcg.source,
    CASE srcg.source
        WHEN 'MONTHLY_GRANT' THEN '每月发放'
        WHEN 'PURCHASE' THEN '购买'
        WHEN 'REWARD' THEN '奖励'
        WHEN 'ADMIN_GRANT' THEN '管理员发放'
        ELSE '未知'
    END AS source_name,
    srcg.grant_month,
    srcg.remark,
    srcg.created_at
FROM sign_in_repair_card_grant srcg
LEFT JOIN users u ON srcg.user_id = u.user_id
ORDER BY srcg.created_at DESC;

-- 用户签到补签综合视图
CREATE OR REPLACE VIEW `user_sign_in_repair_summary_view` AS
SELECT 
    u.user_id,
    u.username,
    sis.current_continuous_days,
    sis.total_sign_ins,
    sis.max_continuous_days,
    sis.last_sign_in_date,
    COALESCE(urcv.normal_card_count, 0) AS normal_card_count,
    COALESCE(urcv.advanced_card_count, 0) AS advanced_card_count,
    COALESCE(urcv.total_card_count, 0) AS total_card_count,
    (SELECT COUNT(*) FROM sign_in_repair_record WHERE user_id = u.user_id AND status = 'SUCCESS') AS total_repairs,
    (SELECT SUM(points_reward) FROM sign_in_repair_record WHERE user_id = u.user_id AND status = 'SUCCESS') AS total_repair_points
FROM users u
LEFT JOIN sign_in_status sis ON u.user_id = sis.user_id
LEFT JOIN user_repair_card_view urcv ON u.user_id = urcv.user_id;

-- 月度补签统计视图
CREATE OR REPLACE VIEW `monthly_repair_statistics_view` AS
SELECT 
    DATE_FORMAT(repair_date, '%Y-%m') AS repair_month,
    COUNT(*) AS total_repairs,
    COUNT(DISTINCT user_id) AS unique_users,
    SUM(points_reward) AS total_points_awarded,
    AVG(continuous_days_after - continuous_days_before) AS avg_continuous_days_gained
FROM sign_in_repair_record
WHERE status = 'SUCCESS'
GROUP BY DATE_FORMAT(repair_date, '%Y-%m')
ORDER BY repair_month DESC;
...
-- 获取用户可用补签卡数量
CREATE FUNCTION `get_user_available_cards`(
    p_user_id BIGINT,
    p_card_type TINYINT
) RETURNS INT
DETERMINISTIC
...
-- 检查日期是否可补签
CREATE FUNCTION `can_repair_date`(
    p_user_id BIGINT,
    p_repair_date DATE
) RETURNS BOOLEAN
DETERMINISTIC
...
BEGIN
    ...
    DECLARE v_has_signed INT;
    DECLARE v_has_repaired INT;
    
    SELECT CAST(config_value AS SIGNED) INTO v_max_days
    FROM sign_in_repair_config
    WHERE config_key = 'max_repair_days_ago' AND is_active = 1;
    
    IF v_max_days IS NULL THEN
        SET v_max_days = 30;
    END IF;
    
    IF p_repair_date >= CURDATE() THEN
        RETURN FALSE;
    END IF;
    
    IF p_repair_date < DATE_SUB(CURDATE(), INTERVAL v_max_days DAY) THEN
        RETURN FALSE;
    END IF;
    
    SELECT COUNT(*) INTO v_has_signed
    FROM sign_in
    WHERE user_id = p_user_id
      AND DATE(sign_in_date) = p_repair_date;
    
    IF v_has_signed > 0 THEN
        RETURN FALSE;
    END IF;
    
    SELECT COUNT(*) INTO v_has_repaired
    FROM sign_in_repair_record
    WHERE user_id = p_user_id
      AND repair_date = p_repair_date
      AND status = 'SUCCESS';
    
    IF v_has_repaired > 0 THEN
        RETURN FALSE;
    END IF;
...
-- 计算补签后连续签到天数
CREATE FUNCTION `calculate_repair_continuous_days`(
    p_user_id BIGINT,
    p_repair_date DATE
) RETURNS INT
DETERMINISTIC
READS SQL DATA
BEGIN
    DECLARE v_continuous_days INT DEFAULT 1;
    DECLARE v_check_date DATE;
    
    SET v_check_date = DATE_SUB(p_repair_date, INTERVAL 1 DAY);
    
    check_loop: WHILE v_check_date >= DATE_SUB(CURDATE(), INTERVAL 365 DAY) DO
        IF NOT EXISTS (
            SELECT 1 FROM sign_in 
            WHERE user_id = p_user_id AND DATE(sign_in_date) = v_check_date
        ) AND NOT EXISTS (
            SELECT 1 FROM sign_in_repair_record 
            WHERE user_id = p_user_id AND repair_date = v_check_date AND status = 'SUCCESS'
        ) THEN
            LEAVE check_loop;
        END IF;
        
        SET v_continuous_days = v_continuous_days + 1;
        SET v_check_date = DATE_SUB(v_check_date, INTERVAL 1 DAY);
    END WHILE;
    
    SET v_check_date = DATE_ADD(p_repair_date, INTERVAL 1 DAY);
    
    forward_loop: WHILE v_check_date <= CURDATE() DO
        IF EXISTS (
            SELECT 1 FROM sign_in 
            WHERE user_id = p_user_id AND DATE(sign_in_date) = v_check_date
        ) OR EXISTS (
    ...
    RETURN v_continuous_days;
END //

-- 获取配置值函数
CREATE FUNCTION `get_repair_config_value`(
...
-- 获取用户本月补签次数
CREATE FUNCTION `get_user_monthly_repairs`(
    p_user_id BIGINT
) RETURNS INT
DETERMINISTIC
READS SQL DATA
BEGIN
    ...
    FROM sign_in_repair_record
    WHERE user_id = p_user_id
      AND YEAR(repair_date) = YEAR(CURDATE())
      AND MONTH(repair_date) = MONTH(CURDATE())
      AND status = 'SUCCESS';
...
-- 执行补签操作存储过程
CREATE PROCEDURE `sp_repair_sign_in`(
    IN p_user_id BIGINT,
    IN p_repair_date DATE,
    OUT p_success BOOLEAN,
    OUT p_message VARCHAR(255)
)
BEGIN
    DECLARE v_card_count INT;
    DECLARE v_continuous_before INT;
    DECLARE v_continuous_after INT;
    DECLARE v_points_ratio DECIMAL(3,2);
    DECLARE v_normal_reward INT;
    DECLARE v_repair_points INT;
    DECLARE v_max_accumulation INT;
    
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;
        SET p_success = FALSE;
        SET p_message = '补签操作失败,请稍后重试';
    END;
    
    START TRANSACTION;
    
    IF NOT can_repair_date(p_user_id, p_repair_date) THEN
        SET p_success = FALSE;
        SET p_message = '该日期不可补签';
        ROLLBACK;
    ELSE
        SET v_card_count = get_user_available_cards(p_user_id, 1);
        
        IF v_card_count <= 0 THEN
            SET p_success = FALSE;
            SET p_message = '补签卡数量不足';
            ROLLBACK;
        ELSE
            UPDATE sign_in_repair_card
            SET quantity = quantity - 1, updated_at = NOW(6)
            WHERE user_id = p_user_id AND card_type = 1
            LIMIT 1;
            
            SET v_continuous_before = 0;
            SET v_continuous_after = calculate_repair_continuous_days(p_user_id, p_repair_date);
            
            SELECT CAST(config_value AS DECIMAL(3,2)) INTO v_points_ratio
            FROM sign_in_repair_config
            WHERE config_key = 'repair_points_ratio' AND is_active = 1;
            
            IF v_points_ratio IS NULL THEN
                SET v_points_ratio = 0.5;
            END IF;
            
            SELECT COALESCE(points_reward, 10) INTO v_normal_reward
            FROM sign_in_config
            WHERE continuous_days <= v_continuous_after AND is_active = 1
            ORDER BY continuous_days DESC
            LIMIT 1;
            
            IF v_normal_reward IS NULL THEN
                SET v_normal_reward = 10;
            END IF;
            
            SET v_repair_points = CEIL(v_normal_reward * v_points_ratio);
            
            INSERT INTO sign_in (user_id, sign_in_date, continuous_days, points_reward, created_at)
            VALUES (p_user_id, p_repair_date, v_continuous_after, v_repair_points, NOW(6));
            
            INSERT INTO sign_in_repair_record (user_id, repair_date, card_type, continuous_days_before, continuous_days_after, points_reward, status, created_at)
            VALUES (p_user_id, p_repair_date, 1, v_continuous_before, v_continuous_after, v_repair_points, 'SUCCESS', NOW(6));
            
            UPDATE sign_in_status
            SET total_sign_ins = total_sign_ins + 1,
                current_continuous_days = GREATEST(current_continuous_days, v_continuous_after),
                max_continuous_days = GREATEST(max_continuous_days, v_continuous_after),
                updated_at = NOW(6)
            WHERE user_id = p_user_id;
            
            SET p_success = TRUE;
            SET p_message = CONCAT('补签成功,获得', v_repair_points, '积分');
...
-- 每月发放补签卡存储过程
CREATE PROCEDURE `sp_grant_monthly_repair_cards`()
BEGIN
    DECLARE v_grant_quantity INT;
    DECLARE v_max_accumulation INT;
    DECLARE v_current_month VARCHAR(7);
    DECLARE v_done INT DEFAULT FALSE;
    DECLARE v_user_id BIGINT;
    
    DECLARE user_cursor CURSOR FOR SELECT user_id FROM sign_in_status;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET v_done = TRUE;
    
    SELECT CAST(config_value AS SIGNED) INTO v_grant_quantity
    FROM sign_in_repair_config
    WHERE config_key = 'monthly_grant_quantity' AND is_active = 1;
    
    IF v_grant_quantity IS NULL THEN
        SET v_grant_quantity = 3;
    END IF;
    
    SELECT CAST(config_value AS SIGNED) INTO v_max_accumulation
    FROM sign_in_repair_config
    WHERE config_key = 'max_card_accumulation' AND is_active = 1;
    
    IF v_max_accumulation IS NULL THEN
        SET v_max_accumulation = 10;
    END IF;
    ...
    read_loop: LOOP
        ...
        IF NOT EXISTS (
            SELECT 1 FROM sign_in_repair_card_grant
            WHERE user_id = v_user_id
              AND source = 'MONTHLY_GRANT'
              AND grant_month = v_current_month
        ) THEN
            INSERT INTO sign_in_repair_card_grant (user_id, card_type, quantity, source, grant_month, remark, created_at)
            VALUES (v_user_id, 1, v_grant_quantity, 'MONTHLY_GRANT', v_current_month, '每月自动发放', NOW(6));
            
            INSERT INTO sign_in_repair_card (user_id, card_type, quantity, source, created_at, updated_at)
            VALUES (v_user_id, 1, v_grant_quantity, 'MONTHLY_GRANT', NOW(6), NOW(6))
            ON DUPLICATE KEY UPDATE
                quantity = LEAST(quantity + v_grant_quantity, v_max_accumulation),
...
-- 清理过期补签卡存储过程
CREATE PROCEDURE `sp_cleanup_expired_cards`()
BEGIN
    UPDATE sign_in_repair_card
    SET quantity = 0, updated_at = NOW(6)
...
-- 每月1日0点发放补签卡事件
CREATE EVENT IF NOT EXISTS `evt_monthly_grant_repair_cards`
ON SCHEDULE EVERY 1 MONTH
STARTS CONCAT(DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 MONTH), '%Y-%m'), '-01 00:00:00')
ON COMPLETION PRESERVE
ENABLE
DO CALL sp_grant_monthly_repair_cards();

-- 每天凌晨1点清理过期补签卡事件
CREATE EVENT IF NOT EXISTS `evt_daily_cleanup_expired_cards`
ON SCHEDULE EVERY 1 DAY
STARTS CONCAT(CURDATE(), ' 01:00:00')
ON COMPLETION PRESERVE
ENABLE
DO CALL sp_cleanup_expired_cards();

-- 每月生成补签统计报告事件
CREATE EVENT IF NOT EXISTS `evt_monthly_repair_statistics`
ON SCHEDULE EVERY 1 MONTH
...
DO
BEGIN
    INSERT INTO sign_in_repair_config (config_key, config_value, description, is_active, created_at)
    VALUES (
        CONCAT('monthly_repair_stats_', DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y%m')),
        (SELECT JSON_OBJECT(
            'total_repairs', COUNT(*),
            'unique_users', COUNT(DISTINCT user_id),
            'total_points', COALESCE(SUM(points_reward), 0)
        ) FROM sign_in_repair_record 
         WHERE DATE_FORMAT(repair_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y-%m')
         AND status = 'SUCCESS'),
        CONCAT(DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 1 MONTH), '%Y年%m月'), '补签统计'),
        1,
        NOW(6)
    )
    ON DUPLICATE KEY UPDATE config_value = VALUES(config_value), updated_at = NOW(6);
END;
...
-- 邮箱验证Token详情视图(关联用户基本信息)
CREATE VIEW `email_token_detail_view` AS
SELECT 
    evt.token_id,
    evt.user_id,
    u.username,
    ...
    evt.validated_at,
    evt.revoked_at,
    evt.created_at AS token_created_at,
    evt.updated_at AS token_updated_at,
    TIMESTAMPDIFF(SECOND, NOW(), evt.expire_at) AS remaining_seconds,
    CASE 
        WHEN evt.status = 1 THEN 'VALIDATED'
        WHEN evt.status = 3 THEN 'REVOKED'
        WHEN evt.expire_at < NOW() THEN 'EXPIRED'
        WHEN evt.status = 0 THEN 'PENDING'
        ELSE 'UNKNOWN'
    END AS effective_status
FROM email_validate_tokens evt
LEFT JOIN users u ON evt.user_id = u.user_id
LEFT JOIN user_profiles up ON evt.user_id = up.user_id;

-- 当前有效/待处理Token视图(未使用且未过期的Token)
CREATE VIEW `email_token_active_view` AS
SELECT 
    evt.*,
    u.username,
    up.nickname,
...
LEFT JOIN user_profiles up ON evt.user_id = up.user_id
WHERE evt.status = 0
AND evt.expire_at > NOW();

-- 邮箱验证统计视图(按用户维度)
CREATE VIEW `email_validate_stats_by_user_view` AS
SELECT 
    evt.user_id,
    u.username,
    up.nickname,
    ...
    SUM(CASE WHEN evt.status = 0 AND evt.expire_at > NOW() THEN 1 ELSE 0 END) AS pending_count,
    SUM(CASE WHEN evt.status = 1 THEN 1 ELSE 0 END) AS validated_count,
    SUM(CASE WHEN evt.status = 2 OR (evt.status = 0 AND evt.expire_at < NOW()) THEN 1 ELSE 0 END) AS expired_count,
    SUM(CASE WHEN evt.status = 3 THEN 1 ELSE 0 END) AS revoked_count,
    MAX(evt.validated_at) AS last_validated_at,
    MAX(evt.created_at) AS last_token_created_at
FROM email_validate_tokens evt
LEFT JOIN users u ON evt.user_id = u.user_id
LEFT JOIN user_profiles up ON evt.user_id = up.user_id
GROUP BY evt.user_id, u.username, up.nickname, up.email;

-- 邮箱验证统计视图(按Token类型维度)
CREATE VIEW `email_validate_stats_by_type_view` AS
SELECT 
    token_type,
    COUNT(*) AS total_count,
    SUM(CASE WHEN status = 0 AND expire_at > NOW() THEN 1 ELSE 0 END) AS pending_count,
    SUM(CASE WHEN status = 1 THEN 1 ELSE 0 END) AS validated_count,
    SUM(CASE WHEN status = 2 OR (status = 0 AND expire_at < NOW()) THEN 1 ELSE 0 END) AS expired_count,
    SUM(CASE WHEN status = 3 THEN 1 ELSE 0 END) AS revoked_count,
    ROUND(SUM(CASE WHEN status = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0) * 100, 2) AS validation_rate
FROM email_validate_tokens
GROUP BY token_type;

-- 邮箱验证历史汇总视图(按邮箱维度聚合操作记录)
CREATE VIEW `email_history_summary_view` AS
SELECT 
    evh.email,
    evh.user_id,
    u.username,
    COUNT(*) AS total_actions,
    SUM(CASE WHEN evh.action_type = 'SEND' THEN 1 ELSE 0 END) AS send_count,
    SUM(CASE WHEN evh.action_type = 'VALIDATE' THEN 1 ELSE 0 END) AS validate_count,
    SUM(CASE WHEN evh.action_type = 'EXPIRE' THEN 1 ELSE 0 END) AS expire_count,
    SUM(CASE WHEN evh.action_type = 'REVOKE' THEN 1 ELSE 0 END) AS revoke_count,
    SUM(CASE WHEN evh.result = 1 THEN 1 ELSE 0 END) AS success_count,
    SUM(CASE WHEN evh.result = 0 THEN 1 ELSE 0 END) AS fail_count,
    ROUND(SUM(CASE WHEN evh.result = 1 THEN 1 ELSE 0 END) / NULLIF(COUNT(*), 0) * 100, 2) AS success_rate,
    MAX(evh.created_at) AS last_action_at,
    MIN(evh.created_at) AS first_action_at,
    COUNT(DISTINCT evh.client_ip) AS distinct_ip_count
FROM email_validate_history evh
LEFT JOIN users u ON evh.user_id = u.user_id
GROUP BY evh.email, evh.user_id, u.username;

-- 待处理/即将过期Token提醒视图
CREATE VIEW `email_pending_reminder_view` AS
SELECT 
    evt.token_id,
    evt.user_id,
    evt.email,
    ...
    evt.expire_at,
    evt.created_at,
    TIMESTAMPDIFF(MINUTE, NOW(), evt.expire_at) AS remaining_minutes,
    CASE 
        WHEN TIMESTAMPDIFF(MINUTE, NOW(), evt.expire_at) <= 5 THEN 'URGENT'
        WHEN TIMESTAMPDIFF(MINUTE, NOW(), evt.expire_at) <= 15 THEN 'WARNING'
        ELSE 'NORMAL'
...
WHERE evt.status = 0
AND evt.expire_at > NOW()
ORDER BY evt.expire_at ASC;

-- 用户邮箱验证完整状态视图(结合用户资料中的邮箱与验证记录)
CREATE VIEW `user_email_verification_view` AS
SELECT 
    u.user_id,
    u.username,
    up.email AS registered_email,
	MAX(CASE WHEN evt.status = 1 AND evt.token_type IN ('REGISTER', 'BIND') THEN 1 ELSE 0 END) AS email_verified,
    MAX(CASE WHEN evt.status = 1 AND evt.token_type IN ('REGISTER', 'BIND') THEN evt.validated_at END) AS last_email_verified_at,
    COUNT(CASE WHEN evt.status = 1 AND evt.token_type IN ('REGISTER', 'BIND') THEN 1 END) AS total_verified_count,
    COUNT(CASE WHEN evt.status = 0 AND evt.expire_at > NOW() THEN 1 END) AS current_pending_tokens,
    MAX(CASE WHEN evt.status = 0 AND evt.expire_at > NOW() THEN evt.token_type END) AS pending_token_type,
    MAX(CASE WHEN evt.status = 0 AND evt.expire_at > NOW() THEN evt.expire_at END) AS pending_token_expire_at
FROM users u
LEFT JOIN user_profiles up ON u.user_id = up.user_id
LEFT JOIN email_validate_tokens evt ON u.user_id = evt.user_id
GROUP BY u.user_id, u.username, up.email;
...
-- 生成邮箱验证Token
CREATE FUNCTION `generate_email_token`() RETURNS VARCHAR(100) NO SQL
BEGIN
    DECLARE v_token VARCHAR(100);
    SET v_token = CONCAT('EVT_', UPPER(REPLACE(UUID(), '-', '')), '_', UNIX_TIMESTAMP());
    RETURN v_token;
END //

-- 检查指定Token是否有效(未使用 + 未过期 + 未撤销)
CREATE FUNCTION `is_email_token_valid`(p_token VARCHAR(100)) RETURNS BOOLEAN
READS SQL DATA
BEGIN
    DECLARE v_exists INT;
    ...
    SELECT status, expire_at INTO v_status, v_expire_at
    FROM email_validate_tokens
    WHERE token = p_token
    LIMIT 1;
...
-- 获取用户指定邮箱的验证状态
CREATE FUNCTION `get_user_email_verify_status`(p_user_id BIGINT, p_email VARCHAR(100)) RETURNS VARCHAR(20)
READS SQL DATA
BEGIN
    DECLARE v_status VARCHAR(20) DEFAULT 'NOT_FOUND';
    DECLARE v_has_validated INT;
    
    SELECT COUNT(*) INTO v_has_validated
    FROM email_validate_tokens
    WHERE user_id = p_user_id
      AND email = p_email
      AND status = 1
      AND token_type IN ('REGISTER', 'BIND');
    
    IF v_has_validated > 0 THEN
...
-- 统计指定邮箱在最近N小时内的发送次数(用于防刷限制)
CREATE FUNCTION `count_recent_email_sends`(p_email VARCHAR(100), p_hours INT) RETURNS INT
READS SQL DATA
BEGIN
    DECLARE v_send_count INT DEFAULT 0;
    
    SELECT COUNT(*) INTO v_send_count
    FROM email_validate_history
    WHERE email = p_email
...
-- 统计指定邮箱在最近N小时内的验证成功次数
CREATE FUNCTION `count_recent_email_validates`(p_email VARCHAR(100), p_hours INT) RETURNS INT
READS SQL DATA
BEGIN
    DECLARE v_count INT DEFAULT 0;
...
-- 获取指定邮箱当前活跃的未使用Token数量
CREATE FUNCTION `get_active_email_token_count`(p_email VARCHAR(100)) RETURNS INT
READS SQL DATA
BEGIN
    DECLARE v_count INT DEFAULT 0;
...
-- 检查指定IP在最近N分钟内的请求次数(用于频率限制)
CREATE FUNCTION `count_ip_email_requests`(p_ip VARCHAR(45), p_minutes INT) RETURNS INT
READS SQL DATA
BEGIN
    DECLARE v_count INT DEFAULT 0;
...
-- 插入Token时自动记录历史(SEND操作)
CREATE TRIGGER `trg_after_email_token_insert`
AFTER INSERT ON `email_validate_tokens`
FOR EACH ROW
BEGIN
    INSERT INTO `email_validate_history`
    (user_id, email, action_type, token_type, result, client_ip, user_agent)
    VALUES
    (NEW.user_id, NEW.email, 'SEND', NEW.token_type, 1, NULL, NULL);
END$$

-- 更新Token状态时自动记录历史(验证成功/过期/撤销)
CREATE TRIGGER `trg_after_email_token_update`
AFTER UPDATE ON `email_validate_tokens`
FOR EACH ROW
BEGIN
    -- 状态从"未使用"变为"已验证", 记录验证成功历史
    IF OLD.status = 0 AND NEW.status = 1 THEN
        INSERT INTO `email_validate_history`
        (user_id, email, action_type, token_type, result, client_ip, user_agent)
        VALUES
        (NEW.user_id, NEW.email, 'VALIDATE', NEW.token_type, 1, NULL, NULL);
    END IF;
    ...
    IF OLD.status <> 3 AND NEW.status = 3 THEN
        ...
        (user_id, email, action_type, token_type, result, client_ip, user_agent)
        VALUES
        (NEW.user_id, NEW.email, 'REVOKE', NEW.token_type, 1, NULL, NULL);
    END IF;
    
    -- Token过期(状态变为已过期或未使用但已超时), 记录过期历史
    IF (OLD.status = 0 AND NEW.status = 2) OR
       (OLD.status = 0 AND NEW.status = 0 AND OLD.expire_at >= NOW() AND NEW.expire_at < NOW()) THEN
        INSERT INTO `email_validate_history`
        (user_id, email, action_type, token_type, result, client_ip, user_agent)
        VALUES
        (NEW.user_id, NEW.email, 'EXPIRE', NEW.token_type, 1, NULL, NULL);
    END IF;
END$$

-- 插入Token前检查同一邮箱的未使用Token数量限制(防止滥发)
CREATE TRIGGER `trg_before_email_token_insert_check`
BEFORE INSERT ON `email_validate_tokens`
FOR EACH ROW
BEGIN
    DECLARE v_active_count INT DEFAULT 0;
    DECLARE v_max_pending INT DEFAULT 5;
    ...
    -- 同一邮箱最多允许5个未使用的pending Token
    IF v_active_count >= v_max_pending THEN
	    SET v_error_msg = CONCAT('该邮箱的待验证Token数量已达上限(', v_max_pending, '), 请等待现有Token过期或先完成验证');
        SIGNAL SQLSTATE '45000'
...
-- 每小时清理过期的Email Token并更新状态
CREATE EVENT IF NOT EXISTS `event_cleanup_expired_email_tokens`
ON SCHEDULE EVERY 1 HOUR
STARTS TIMESTAMP(DATE_ADD(NOW(), INTERVAL 1 HOUR))
COMMENT '每小时清理并标记过期的邮箱验证Token'
DO BEGIN
    -- 将过期但状态仍为"未使用"的Token标记为"已过期"
    UPDATE email_validate_tokens
    SET status = 2,
        updated_at = NOW(6)
    WHERE status = 0
      AND expire_at < NOW(6);
    ...
    -- 记录批量过期到历史表
    INSERT INTO email_validate_history
    (user_id, email, action_type, token_type, result, error_message)
    SELECT user_id, email, 'EXPIRE', token_type, 1, '定时任务自动过期'
    FROM email_validate_tokens
    WHERE status = 2
      AND DATE(updated_at) = CURRENT_DATE()
      AND expire_at < NOW(6);
...
-- 每日生成邮箱验证统计摘要
CREATE EVENT IF NOT EXISTS `event_email_validate_daily_statistics`
ON SCHEDULE EVERY 1 DAY
STARTS TIMESTAMP(CURDATE() + INTERVAL 1 DAY, '02:00:00')
COMMENT '每日生成邮箱验证统计摘要'
DO BEGIN
    -- 该事件用于配合应用层的每日统计报表
    -- 可扩展: 插入到统计汇总表或发送告警邮件
    ...
    SELECT 
        NULL AS user_id,
        'SYSTEM@daily.stats' AS email,
        'SEND' AS action_type,
        'OTHER' AS token_type,
        1 AS result,
        CONCAT(
            'Daily Stats | TotalTokens:', COUNT(*),
            ' | Pending:', SUM(CASE WHEN status = 0 AND expire_at > NOW() THEN 1 ELSE 0 END),
            ' | Validated:', SUM(CASE WHEN status = 1 THEN 1 ELSE 0 END),
            ' | Expired:', SUM(CASE WHEN status = 2 THEN 1 ELSE 0 END),
            ' | Revoked:', SUM(CASE WHEN status = 3 THEN 1 ELSE 0 END)
        ) AS error_message
    FROM email_validate_tokens
    WHERE DATE(created_at) = CURRENT_DATE() - INTERVAL 1 DAY;
END$$

-- 每6小时清理30天前的历史记录(保留近期数据, 释放存储空间)
CREATE EVENT IF NOT EXISTS `event_cleanup_old_email_history`
ON SCHEDULE EVERY 6 HOUR
STARTS TIMESTAMP(DATE_ADD(NOW(), INTERVAL 6 HOUR))
COMMENT '每6小时清理30天前的邮箱验证历史记录'
DO BEGIN
    DELETE FROM email_validate_history
...
-- 补签配置
INSERT INTO `sign_in_repair_config` (`config_key`, `config_value`, `description`, `is_active`) VALUES
('monthly_grant_quantity', '3', '每月发放补签卡数量', 1),
('max_repair_days_ago', '30', '最大可补签天数(距今天数)', 1),
('repair_points_ratio', '0.5', '补签积分比例(相对于正常签到)', 1),
('max_card_accumulation', '10', '补签卡最大累积数量(0表示无限制)', 1),
('enable_repair_card_expiry', 'false', '是否启用补签卡过期', 1),
('card_expiry_months', '3', '补签卡过期月数(启用过期时有效)', 1)
;

-- 邮件配置
INSERT IGNORE INTO `system_configs` (`config_key`, `config_value`, `config_type`, `description`, `is_public`) VALUES 
('email.validate.expire.minutes', '1440', 'INTEGER', '邮箱验证链接有效期（分钟），默认24小时', 0),
('email.validate.max.send.count', '10', 'INTEGER', '同一邮箱每日最大发送次数限制', 0),
('email.sender.name', '小咪楂系统通知', 'STRING', '邮件发送者显示名称', 1),
('email.front.redirect.url', '', 'STRING', '前端邮箱验证回调地址', 1)
;
