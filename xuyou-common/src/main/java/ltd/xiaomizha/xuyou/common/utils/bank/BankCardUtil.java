...
public class BankCardUtil {
    ...
    private static final List<BankCardRule> BANK_CARD_RULES = new ArrayList<>();

    private static final String CARD_TYPE_DC = "DC";
    private static final String CARD_TYPE_CC = "CC";
    private static final String CARD_TYPE_SCC = "SCC";
    private static final String CARD_TYPE_PC = "PC";

    private static final String TYPE_NAME_DC = "借记卡";
    private static final String TYPE_NAME_CC = "信用卡";
    private static final String TYPE_NAME_SCC = "准贷记卡";
    private static final String TYPE_NAME_PC = "预付费卡";

    static {
        initBankCardRules();
    }

    private static class BankCardRule {
        private final String bankCode;
        private final String bankName;
        private final Pattern pattern;
        private final String cardType;

        public BankCardRule(String bankCode, String bankName, String pattern, String cardType) {
     ...
     * @return 银行卡信息, 未找到返回null
     */
    public static BankCardInfo searchCardBin(String bankCardNo) {
        if (bankCardNo == null || bankCardNo.trim().isEmpty()) {
            return null;
        }
        String cardNo = bankCardNo.trim();

        for (BankCardRule rule : BANK_CARD_RULES) {
            if (Pattern.matches(String.valueOf(rule.getPattern()), cardNo)) {
                return BankCardInfo.success(rule.getBankName(), rule.getBankCode(), rule.getCardType(), getCardTypeName(rule.getCardType()));
            }
        }
        return null;
    }
     ...
     * @return 验证结果
     */
    public static BankCardInfo validateCardInfo(String bankCardNo) {
        if (bankCardNo == null || bankCardNo.trim().isEmpty()) {
            return BankCardInfo.fail("02", "银行卡号不能为空");
        }
        String cardNo = bankCardNo.trim();

        if (!cardNo.matches("\\d+")) {
            return BankCardInfo.fail("02", "银行卡号格式错误");
        }

        BankCardInfo info = searchCardBin(cardNo);
        if (info == null) {
            return BankCardInfo.fail("01", "找不到该银行卡号");
        }

        info.setCardBin(extractCardBin(cardNo));
        info.setCardLength(cardNo.length());
        return info;
    }
     ...
     * 获取卡类型名称
     */
    private static String getCardTypeName(String cardType) {
        return switch (cardType) {
            case CARD_TYPE_DC -> TYPE_NAME_DC;
            case CARD_TYPE_CC -> TYPE_NAME_CC;
            case CARD_TYPE_SCC -> TYPE_NAME_SCC;
            case CARD_TYPE_PC -> TYPE_NAME_PC;
            default -> "未知";
        };
    }
     ...
     * 提取卡BIN (前6位)
     */
    private static String extractCardBin(String cardNo) {
        if (cardNo.length() >= 6) {
     ...
     * 初始化银行卡规则数据
     */
    private static void initBankCardRules() {

        addRule("PSBC", "中国邮政储蓄银行",
        ...
        addRule("CCB", "中国建设银行",
                ...
                CARD_TYPE_DC);
        addRule("CCB", "中国建设银行",
                "^(421349|434061|434062|524094|526410|552245|621080|621082|421466|621488|621499|622966|622988|622382|621487|621083|621084|620107)\\d{10}$",
                CARD_TYPE_DC);
        addRule("CCB", "中国建设银行",
        ...
        addRule("BOCOM", "交通银行",
                ...
                CARD_TYPE_DC);
        addRule("BOCOM", "交通银行",
                "^(405512|601428|622258|622259)\\d{11}$",
                CARD_TYPE_DC);
        addRule("BOCOM", "交通银行",
                "^(49104|53783)\\d{11}$",
                CARD_TYPE_CC);
        ...
        addRule("GHBANK", "广东华兴银行",
                "^(621469|621625)\\d{13}$",
                CARD_TYPE_DC);

        addRule("SRCB", "深圳农村商业银行",
                "^(622128|622129|623035)\\d{10}$",
                CARD_TYPE_DC);

        addRule("GZRCB", "广州农村商业银行",
                "^(909810|940035|621522|622439)\\d{12}$",
                CARD_TYPE_DC);

        addRule("DRCBCL", "东莞农村商业银行",
        ...
        addRule("HNRCU", "湖南省农村信用社联合社",
                "^(622906|628386|625519|625506)\\d{10}$",
                CARD_TYPE_DC);

        addRule("JXNX", "江西省农村信用社联合社",
                "^(621592)\\d{10}$",
                CARD_TYPE_DC);
        addRule("JXNX", "江西省农村信用社联合社",
                "^(628392)\\d{10}$",
        ...
        addRule("HSSBANK", "衡水市商业银行",
                "^(621239|623068)\\d{13}$",
                CARD_TYPE_DC);

        addRule("CQNYYH", "重庆南川石银村镇银行",
                "^(621653004)\\d{10}$",
                CARD_TYPE_DC);

        addRule("HNRCC", "湖南省农村信用社联合社",
        ...
        addRule("LFYDQYXZH", "临汾市尧都区农村信用合作联社",
                "^(628382|625158)\\d{10}$",
                CARD_TYPE_CC);
        ...
        addRule("HZCB", "宜昌市商业银行",
                "^(940055)\\d{11}$",
                CARD_TYPE_DC);
        addRule("HZCB", "杭州市商业银行",
                "^(603506)\\d{13}$",
                CARD_TYPE_DC);
        addRule("SZSBANK", "苏州市商业银行",
                "^(622399|940043)\\d{11}$",
                CARD_TYPE_DC);
        ...
        addRule("JZCB", "焦作市商业银行",
                "^(622394|940025)\\d{10}$",
                CARD_TYPE_DC);

        addRule("ZJCBBANK", "镇江市商业银行",
                "^(621245)\\d{10}$",
                CARD_TYPE_DC);
        ...
        addRule("MACAUDB", "澳门BDA",
                "^(621274|621324)\\d{13}$",
                CARD_TYPE_DC);
    }

    private static void addRule(String bankCode, String bankName, String pattern, String cardType) {
        BANK_CARD_RULES.add(new BankCardRule(bankCode, bankName, pattern, cardType));
    }
